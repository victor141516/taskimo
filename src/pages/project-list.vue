...
const arrowHandler = (offset: 1 | -1) => {
  if (!currentFocusedProject) {
    ...
    const index = projectsWithDomRefs.findIndex(e => e.project.id === currentFocusedProject!.project.id)
    ...
    >
      <router-link :to="`/projects/${project.id}`" class="w-full">
        <div class="w-full">
          ...
          <div class="badge ml-2 success">{{ project.items.length }}</div>
        </div>
        <button class="btn btn-error btn-sm" @click.prevent="deleteProject(project.id)">Delete</button>
      </router-link>
    </li>
  </ul>
  <div>
    ...
    <form class="flex items-center px-5" v-if="showNewProjectMenu" @submit="onNewProjectSave">
      <input
